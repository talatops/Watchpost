    ...
    <div className="h-screen bg-darkBg flex overflow-hidden">
      ...
      <aside className="w-60 bg-darkCard border-r border-darkBorder flex flex-col flex-shrink-0">
        ...
        <nav className="flex-1 overflow-y-auto p-3 space-y-4">
          {groups.map(group => (
            <div key={group}>
              <p className="text-[10px] font-semibold text-gray-600 uppercase tracking-widest px-3 mb-1">{group}</p>
              {NAV.filter(n => n.group === group).map(item => {
                ...
                return (
                  ...
                  >
                    ...
                    {item.label}
                    {active && (
                      <span className="ml-auto w-1.5 h-1.5 rounded-full bg-accentCyan" />
                    )}
                  </button>
                );
              })}
        ...
        <div className="p-3 border-t border-darkBorder">
          <div className="flex items-center gap-3 p-3 rounded-xl bg-darkBg border border-darkBorder mb-2">
